...
class Jacobi2DOptGenerator(Jacobi2DBaseGenerator):
    def __init__(self, experiment, testing=False):
        super().__init__(experiment, "jacobi2dopt")
    ...
    def get_size_to_allocate_i(self, configuration):
        return Jacobi2DOptGenerator.get_size_to_allocate(configuration)
    ...
    def build(self, configuration):
        ...
        portion_unroll_write_values = portion_unrolls_write * self.experiment.constants.simd_vec_values
        portion_unroll_write_bytes = portion_unrolls_write * self.experiment.constants.simd_vec_bytes
        unalignment_factor = configuration["unalignment_factor"]
        trueX = self.get_true_N(X - 2, [stride_unrolls, stride_unrolls_write], [portion_unrolls, portion_unrolls_write], unalignment_factor=unalignment_factor)
        ...
        W = ((trueX - 2) ** 2) // stride_unrolls_write
        ...
        with CodeContext(self, stride_unrolls, portion_unrolls, N, trueN, suffix=configuration["suffix"], test_function_configuration=(self.test, configuration) if self.testing else None) as cc:
            ...
            cc.add_statement("subq", f"${2 * trueX * self.experiment.constants.dtype_size_bytes}", offset)
            ...
            with For(cc, f"${S}"):
                ...
                # Writeback code
                cc.add_statement("subq", offset, I)
                cc.add_statement("subq", offset, O)

                with For(cc, f"${(trueX - 2) // stride_unrolls_write}"):
                    with For(cc, f"${(trueX - 2) // portion_unroll_write_values}"):
                        for i in range(portion_unrolls_write):
                            for j in range(stride_unrolls_write):
                                ymm = f"%{cc.get_register(register_set='simd', variable='vec')}"

                                offset_c = self.z((j + 1) * trueX * self.experiment.constants.dtype_size_bytes + i * self.experiment.constants.simd_vec_bytes + self.experiment.constants.dtype_size_bytes)

                                cc.add_statement("vmovups", f"{offset_c}({O})", ymm)
                                cc.add_statement("vmovups", ymm, f"{offset_c}({I})")

                                cc.unset_variable("vec")

                        cc.add_statement("addq", f"${portion_unroll_write_bytes}", I)
                        cc.add_statement("addq", f"${portion_unroll_write_bytes}", O)

                    cc.add_statement("addq", f"${((stride_unrolls_write - 1) * trueX + 2) * self.experiment.constants.dtype_size_bytes}", I)
                    cc.add_statement("addq", f"${((stride_unrolls_write - 1) * trueX + 2) * self.experiment.constants.dtype_size_bytes}", O)
        ...
        return (trueN, N, {"trueX": trueX, "S": S, "prebuild_side": trueX})
