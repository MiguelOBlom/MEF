...
class GemverOuterBaseGenerator(ComputeBaseGenerator):
    ...
    def test(self, configuration, test_data_dir):
        ...
        self.write_test_input(test_data_dir, a, u1, v1, u2, v2, remainder)

        for i in range(trueP):
            for j in range(trueP):
                a[i][j] += u1[i] * v1[j] + u2[i] * v2[j]

        self.write_test_output(test_data_dir, a, u1, v1, u2, v2, remainder)
